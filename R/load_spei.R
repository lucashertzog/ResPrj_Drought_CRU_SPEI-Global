load_spei <- function(
  ...
  dat_spei <- readRDS(file.path(indir, "spei1950.rds"))
  setDT(dat_spei)
  
  setnames(dat_spei, "value", "spei_value")
  ...
  spei <- spei[, .(adm0, adm1, adm2, date, spei_value, period48, period24)]
  ...
  mean_adm1 <- adm1_spei[, .(spei_value = mean(spei_value)),by = .(adm0, adm1, date, period24, period48)]
